...
@main.command()
@click.argument('input-bindings', type=click.Path())
...
def mosaic(input_bindings, output_vaccine, mosaics, max_aminoacids, max_epitopes):
    ...
    with open(output_vaccine, 'w') as f:
        writer = csv.writer(f)
        writer.writerow(('mosaic', 'index', 'epitope'))
        for i, mosaic in enumerate(result):
