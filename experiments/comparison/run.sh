...
do
    ...
    if [ ! -f "$RESDIR/made-proteins.fasta" ]; then
        python data_preparation.py -v random-sequences "$BASEDIR/resources/hiv1-bc-env.fasta" 300 "$RESDIR/made-proteins.fasta"
    else
        echo "Not overwriting proteins!"
    fi;

    if [ ! -f "$RESDIR/made-alleles.csv" ]; then
        cp "$BASEDIR/resources/alleles.csv" "$RESDIR/made-alleles.csv"
    else
